...
function ENT:Impact(tr1, phys, bHull)
	...
	self:FireBullets({
		...
		Callback = function(attacker, tr, dmgInfo)
			...
			for i = 1, 6, 1 do 
				local napalm = ents.Create("sdrk_napalm_cluster")
				napalm:SetPos(self:GetPos() + tr.HitNormal * 5)
				napalm:Spawn()
				napalm.Owner = self:GetOwner()
				local phys = napalm:GetPhysicsObject()
				phys:SetVelocity(tr.HitNormal * math.random(100, 300))
			end
